...
class PushNotificationHandlerTests(unittest.TestCase):
    ...
    def test_0_register(self):
        self.PN_helper_v2.register(TEST_TOKEN_0, TEST_BCHAT_ID, TEST_DEVICE_TYPE)
        test_device_in_cache = self.database_helper.get_device(TEST_BCHAT_ID)
        ...
        self.PN_helper_v2.register(TEST_TOKEN_1, TEST_BCHAT_ID, TEST_DEVICE_TYPE)
        test_device_in_cache = self.database_helper.get_device(TEST_BCHAT_ID)
    ...
    def test_1_unregister(self):
        ...
        test_device_in_cache = self.database_helper.get_device(TEST_BCHAT_ID)
        self.assertEqual(test_bchat_id, TEST_BCHAT_ID)
    ...
    def test_2_subscribe_closed_group(self):
        self.PN_helper_v2.subscribe_closed_group(TEST_CLOSED_GROUP_ID, TEST_BCHAT_ID)
        ...
        self.PN_helper_v2.subscribe_closed_group(TEST_CLOSED_GROUP_ID, TEST_BCHAT_ID_1)
        test_closed_group_in_cache = self.database_helper.get_closed_group(TEST_CLOSED_GROUP_ID)
        self.assertTrue(TEST_BCHAT_ID_1 in test_closed_group_in_cache.members)
        self.assertEqual(len(test_closed_group_in_cache.members), 2)

    def test_3_unsubscribe_closed_group(self):
        self.PN_helper_v2.unsubscribe_closed_group(TEST_CLOSED_GROUP_ID, TEST_BCHAT_ID_1)
        test_closed_group_in_cache = self.database_helper.get_closed_group(TEST_CLOSED_GROUP_ID)
        self.assertEqual(len(test_closed_group_in_cache.members), 1)
        self.assertFalse(TEST_BCHAT_ID_1 in test_closed_group_in_cache.members)

    def test_4_send_push_notification(self):
        test_message = {'send_to': TEST_BCHAT_ID,
                        'data': TEST_DATA}
    ...
    def test_6_register_legacy_groups_only(self):
        ...
        self.PN_helper_v2.register_legacy_groups_only(TEST_TOKEN_0, TEST_BCHAT_ID, TEST_DEVICE_TYPE, [TEST_CLOSED_GROUP_ID])
        test_device_in_cache = self.database_helper.get_device(TEST_BCHAT_ID)
